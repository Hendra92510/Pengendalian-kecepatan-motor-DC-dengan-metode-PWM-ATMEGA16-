...
// Timer Period: 43,52 ms
// Output Pulse(s):
// OC0 Period: 43,52 ms Width: 0 us
...
while (1)
      ...
      //program pengurangan kecepatan
        if(!PINB.0 && OCR0 != 0x00){
           OCR0 = OCR0 - 15; 
        }
        else if(!PINB.0 && OCR0 == 0x00){
            PORTB.4 = 1; //INDIKATOR HIGH
        }
      //program penambahan kecepatan
        else if(!PINB.1 && OCR0 != 0xFF){
            OCR0 = OCR0 + 15;  
        }                  
        else if(!PINB.1 && OCR0 == 0xFF){
            PORTB.5 = 1; //INDIKATOR HIGH
        }
        else{
            PORTB.4 = 0;
            PORTB.5 = 0; //INDIKATOR MATI
        }
        delay_ms(500);
      }
}
